...
const UNIT_DEFS = {
  ...
  warship:{ id:'warship', name:'Bateau militaire', short:'Bateau', size:3, qty:2, domain:'sea', icon:'▰' },
  tank:{ id:'tank', name:'Tank', short:'Tank', size:2, qty:2, domain:'land', icon:'▣' },
...
function renderHome(){
  app.innerHTML = `<div class="home">
    ...
    <section class="forms">
      <div class="card"><h2>Créer une partie</h2><div class="field"><label>Nom du joueur</label><input id="pname" value="${esc(playerName)}"></div><div class="mapInfo">Carte : Île Forteresse — 10×5, avec 6 colonnes mer et 4 colonnes terre.</div><br><button class="green" onclick="createGame()">Créer la partie</button></div>
...
function renderLobby(){
  ...
  app.innerHTML = `<div class="lobby">
    ...
    <div class="card versus"><div class="playerbox"><div class="avatar">⚓</div><h2>${esc(p1?.name||'En attente')}</h2>${renameInput(p1)}</div><h1>VS</h1><div class="playerbox red"><div class="avatar">🛡️</div><h2>${esc(p2?.name||'En attente')}</h2>${renameInput(p2)}</div></div>
    <div class="card"><h2>Règles de la partie</h2><p>Chaque joueur place : 1 porte-avion de 4 cases, 2 bateaux militaires de 3 cases, 2 tanks de 2 cases et 2 militaires de 1 case.</p><p>La carte est identique pour les deux joueurs : mer à gauche, terre à droite. Les bateaux vont uniquement dans l’eau ; tanks et militaires uniquement sur terre.</p><div class="row"><button class="green" onclick="startPlacement()" ${state.players.length<2?'disabled':''}>Démarrer la partie</button><button class="ghost" onclick="location.reload()">Retour menu</button></div></div>
  </div>`;
}
...
function unitPicker(){
  return UNIT_ORDER.map((id,idx)=>{
    ...
    return `<div class="unitCard ${selectedType===u.id?'active':''}" onclick="selectUnit('${u.id}')"><div class="unitIcon spriteIcon ${u.id}">${unitSprite(u.id, u.domain==='sea'?'h':'v')}</div><b>${u.name} <span class="kbd">${idx+1}</span></b><span>${placedCount(u.id)} / ${u.qty} placé(s)</span><small>${u.size} case(s) • ${u.domain==='sea'?'eau':'terre'}</small></div>`;
  }).join('');
}
...
window.placeAt=placeAt; window.submitPlacement=submitPlacement; window.clearPreview=clearPreview;
...
function renderCombat(){
  ...
  app.innerHTML = `<div class="combat ${myTurn?'your-turn':''}">
    ...
    <div class="battleGrid">
      <aside class="card side"><h3>Votre base</h3>${miniBoard(state.myBoard)}<div class="legend"><span><b class="dot water"></b> Mer</span><span><b class="dot land"></b> Terre</span><span>✕ unité touchée</span><span>• raté</span></div><div style="margin-top:12px;font-size:13px;color:#8aadc7">Unités détruites : <b style="color:#ff8888">${allDestroyed}</b> / 7</div></aside>
...
function enemyBoardHtml(){
  return `<div class="board battleBoard enemyBoard" onmouseout="clearPreview()">${state.enemyKnown.flat().map(cell=>{
    ...
    return `<div data-r="${cell.r}" data-c="${cell.c}" class="cell ${cell.terrain} ${cell.hit?(cell.knownUnit?'hit':'miss'):'fog'} ${selectedShot&&selectedShot.r===cell.r&&selectedShot.c===cell.c?'selected':''} ${previewClass}" onclick="selectShot(${cell.r},${cell.c})" onmouseenter="previewShot(${cell.r},${cell.c})"><span class="coord">${coord(cell.r,cell.c)}</span>${cell.c===6?'<span class="coast"></span>':''}${cell.hit?(cell.knownUnit?'🔥':'•'):''}</div>`;
  }).join('')}</div>`;
}
...
function renderEnded(){
  ...
  const statRows = state.players.map(p => {
    ...
    return `<div class="statCard ${p.id===state.winner?'winnerStat':''}"><div><h3>${esc(p.name)}</h3>${winnerBadge}</div><div class="statGrid"><span>Tirs totaux</span><b>${st.shots}</b><span>Touches</span><b>${st.hits}</b><span>Ratés</span><b>${st.misses}</b><span>Unités coulées/détruites</span><b>${st.sunk}</b></div></div>`;
  }).join('');
  app.innerHTML = `<div class="victory card"><div class="victoryInner"><h1>${state.winner===state.me?'VICTOIRE !':'DÉFAITE'}</h1><h2>Le joueur gagnant est <strong>${esc(winner?.name||'Un joueur')}</strong></h2><div class="statsPanel"><h3>Résumé de la partie</h3><div class="statsCards">${statRows}</div></div><div class="row" style="justify-content:center"><button class="green" onclick="location.reload()">Rejouer</button><button class="ghost" onclick="location.reload()">Retour menu</button></div></div></div>`;
}
...
function boardHtml(board,{mode}){
  ...
  return `<div class="board placementBoard" onmouseout="clearPreview()">${board.flat().map(cell=>{
    ...
    const previewClass = isPreview ? (previewCells._valid ? 'valid-preview' : 'invalid-preview') : '';
    return `<div class="cell ${cell.terrain} ${p?'occupied':''} ${canPlaceHere?'can-place':''} ${previewClass}" onclick="${mode==='place'?`placeAt(${cell.r},${cell.c})`:''}" onmouseenter="${mode==='place'?`previewPlace(${cell.r},${cell.c})`:''}"><span class="coord">${coord(cell.r,cell.c)}</span>${cell.c===6?'<span class="coast">▐</span>':''}${p?`<div class="unitPlaced spriteUnit ${p.type} ${p.orientation}">${unitSprite(p.type,p.orientation)}<small>${UNIT_DEFS[p.type].short}</small></div>`:''}</div>`;
  }).join('')}</div>`;
}
...
function miniBoard(board){ return `<div class="miniBoard">${board.flat().map(c=>`<div data-r="${c.r}" data-c="${c.c}" class="miniCell ${c.terrain} ${c.hit?c.unitId?'hit':'miss':''}"><span>${c.hit?(c.unitId?'✕':'•'):c.unitId?unitMiniFor(c.unitId):''}</span></div>`).join('')}</div>`; }

function unitSprite(type, orientation='h'){
  const cls = `sprite ${type} ${orientation}`;
  if(type==='carrier') return `<div class="${cls}"><i></i><i></i><i></i><i></i></div>`;
  if(type==='warship') return `<div class="${cls}"><i></i><i></i><i></i></div>`;
  if(type==='tank') return `<div class="${cls}"><i></i><i></i></div>`;
  return `<div class="${cls}"><i></i></div>`;
}
function unitMiniFor(unitId){
  const type = String(unitId).split('-')[0];
  return `<b class="miniSprite ${type}"></b>`;
}
