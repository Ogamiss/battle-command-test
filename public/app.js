...
function startTicker() {
  clearInterval(tick);
  tick = setInterval(() => { if (state?.timerEndsAt) render(); }, 1000);
}
startTicker();
...
function clearPreview() { previewCells = []; }
...
function selectShot(r, c) {
  if (state.turn !== state.me) return;
  if (state.enemyKnown[r][c].hit) return showToast('Case déjà bombardée.');
  selectedShot = { r, c };
  render();
}
...
function renderEnded() {
  ...
  app.innerHTML = `<div class="victory card"><div><h1>${isVictory ? 'VICTOIRE !' : 'DÉFAITE'}</h1><h2>${esc(winner?.name || 'Un joueur')} a remporté la partie.</h2><p style="color:#8aadc7">Tirs effectués : ${state.myShots.length} · Précision : ${Math.round(state.myShots.filter(s => s.hit).length / Math.max(1, state.myShots.length) * 100)}%</p><div class="row" style="justify-content:center;margin-top:16px"><button class="green" onclick="location.reload()">Rejouer</button><button class="ghost" onclick="location.reload()">Retour menu</button></div></div></div>`;
}
...
function miniBoard(board) {
  return `<div class="miniBoard">${board.flat().map(c => `<div data-r="${c.r}" data-c="${c.c}" class="miniCell ${c.terrain} ${c.hit ? c.unitId ? 'hit' : 'miss' : ''}"><span>${c.hit ? (c.unitId ? '✕' : '•') : c.unitId ? unitIconFor(c.unitId) : ''}</span></div>`).join('')}</div>`;
}
